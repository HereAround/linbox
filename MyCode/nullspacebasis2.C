...
int main (int argc, char **argv)
{
    ...
    // We expect no arguments - hard coded path to filestream from input.sms which is located in the folder where this executable is located
	if ( argc >  1 ) {
    ...
    // (2) Read matrix
	std::ifstream input ("input.sms");
	if (!input) {
        std::cerr << "Error opening matrix file input.sms" << std::endl; return -1; 
    }
        ...
        std::string s;
        iss >> p1 >> p2 >> s;
        std::cerr << "Number of rows: " << p1 << std::endl;
        std::cerr << "Number of columns: " << p2 << std::endl;
        
        // Initialise sparse matrix
        SparseMatrix<Field, SparseMatrixFormat::SparseSeq > B (F,p1,p2);
        ...
        while (std::getline(input, line))
        {
            ...
            // check if we have not yet reached the end of the file
            if ( p1 != 0 || p2 != 0 || p3 != 0 ) {
                
                // set entries
                Field::Element v;
                F.init(v, p3);
                B.setEntry(p1-1, p2-1, v);
